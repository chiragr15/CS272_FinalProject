...
def evaluate(model_dir: str, obs_type: str, episodes: int=500, seed: int=0):
    ...
    for _ in trange(episodes):
        ...
        while not done:
            ...
            ep_ret += float(rew); done = terminated or truncated
...
if __name__ == "__main__":
    ...
    out = os.path.join(run_dir, f"ID6_Merge_LidarObs_returns.npy" if args.obs=="LidarObservation"
                                 else f"ID8_Merge_GrayscaleObs_returns.npy")
    np.save(out, R)
